# 👋 Hello! I'm Patric Camara Leal

🎓 **Information Systems student** at **Federal University of Santa Maria (UFSM), Brazil**.

💻 **Focused on:**
- Web Development (PHP, JavaScript, Node.js, Laravel, React)
- IoT & Embedded Systems (ESP32, Arduino)
- 3D Printing & Firmware (Marlin, Klipper, Prusa)
- Machine Learning & Data Science
- Open Source & Open Hardware

🌐 **Portfolio / Website:** [https://PatricLeal.github.io](https://PatricLeal.github.io)

📫 **Connect with me:**
- [GitHub](https://github.com/PatricLeal)
- [LinkedIn](https://linkedin.com/in/patric-camara-leal-4)

---

🚀 Always exploring new technologies and building solutions for real-world problems!
